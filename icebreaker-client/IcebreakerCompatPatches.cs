    ...
    internal static class Patch_DayVisionOnLitShip
    {
        ...
        private static void Postfix(LookSensor __instance)
        {
            ...
            try
            {
                ...
                float k = 1f;
                var ai = bo.AIData;
                if (ai == null || !ai.IsInside)
                    k = __instance.WeatherVisibilityK(look.RAIN_DEBUFF_MAXVISIBILITY_MULTIPLYER,
                        look.FOG_DEBUFF_MAXVISIBILITY_MULTIPLYER, out _, out _);
                float dayDist = Mathf.Clamp(baseDist * k, look.MINIMUM_VISIBLE_DIST, 9999f);
